  ...
  return (
     <div className="w-[500px] bg-[#262626] p-4 rounded-lg mb-4">
       <div className="flex items-center gap-5">
           <button className='bg-[#f6b100] p-4 text-xl font-bold rounded-lg'>{getAvatarName(order.customerDetails.name)}</button>
               ...
               <div className="flex flex-col items-end gap-2">
                  {
                    order.orderStatus === "Ready" ? (
                        <>
                         <p className="text-yellow-600 bg-[#4a452e] px-2 py-1 rounded-lg "><FaCircle className='inline mr-2'/>{order.orderStatus}</p></> ):
           ...
           <div className="flex items-center justify-between mt-4">
            ...
            <p className="text-[#f5f5f5] text-lg font-semibold">Nrs. {order.bills.totalWithTax.toFixed(2)}</p>
           </div>
       </div>
  )
}
